...
export default function NetworkAdminDashboard() {
  ...
  if (!loaded) return <div >INITIALIZING IT KERNEL...</div>;
    ...
    return (
      <div key={d.id} style={{ ...S.deviceBox, border: `1px solid ${borderCol}`, boxShadow: isCritical && isTerminal ? '0 0 10px rgba(248,81,73,0.3)' : 'none' }}>
        <div >
          ...
          <Badge variant={d.status === 'ONLINE' ? 'success' : d.status === 'OFFLINE' ? 'error' : 'warning'}>{d.status}</Badge>
        </div>
        ...
        <div style={{ marginTop: 'auto', paddingTop: '12px', borderTop: `1px solid ${isTerminal ? 'rgba(63,185,80,0.1)' : '#e2e8f0'}`, display: 'flex', justifyContent: 'flex-end' }}>
          <Button 
            variant={isTerminal ? 'secondary' : 'secondary'}
        ...
        <div >
          <Card style={S.card}>
            <div >
              ...
              <Badge variant="default">Total: {network.devices.length}</Badge>
            </div>
        ...
        <div >
          ...
          <Card style={S.card}>
            ...
            <div >
              <div >
                <span>Status</span>
                <Badge variant={network.dbStatus === 'AWAKE' ? 'success' : network.dbStatus === 'ASLEEP' ? 'error' : 'warning'}>
                  {network.dbStatus}
                </Badge>
              </div>
              ...
              <div >
                <Button 
                  variant={isTerminal ? (network.dbStatus === 'ASLEEP' ? 'success' : 'secondary') : 'primary'}
