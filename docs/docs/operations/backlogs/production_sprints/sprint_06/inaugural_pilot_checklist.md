# "Purely Doubles Inaugural" - Live Event Launch Checklist

This document tracks the final, highly specific set of environmental and physical checklists required to cross the finish line between the stabilized codebase and the live "Purely Doubles Inaugural" pilot.
...
- [ ] **Database Mirror Check:**
  - Promote local SQLite/Prisma development schema to production-grade cloud PostgreSQL (e.g., Supabase).
...
Run structural dress rehearsals at the physical venue.
...
- [ ] **The Broadcaster OBS / vMix Stencil Binding:**
  - Map video director's encoders (like OBS Studio) to reactive overlay endpoints (`/api/broadcast/highlights/latest`).
...
Staff agents must execute a simulated **Pre-Flight "Break-Glass" Exercise**.
...
- [ ] **The Offline-First Stress Test:**
  ...
  - Verify the PWA local caching mechanisms hold the point variations safely without losing operational states.
  - Verify that upon reconnecting, the client engine accurately "drains" the data queue chronologically back to the PostgreSQL database fortress using `offlineVersion` checks without causing state collisions.
...
Once these vectors pass their verification checkpoints, execute the following sequence on tournament morning:
