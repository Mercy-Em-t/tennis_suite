...
path = r'd:\TENNIS SUITE\src\app\sandbox\host\page.tsx'
...
lines = [
    ...
    "  id: 'sandbox-tournament-001',",
    "  name: 'ATP Nairobi Open 2026',",
    "  location: 'Kasarani Indoor Arena',",
    "};",
    "",
    "const MOCK_TEAMS = [",
    "  { id: 't1', franchiseName: 'Hawkeye Raiders', categories: '[\"Open\"]', isLate: false, players: [{ id: 'p1', name: 'Carlos Mejia', email: 'carlos@play.com' }, { id: 'p2', name: 'David Osei', email: 'dosei@play.com' }] },",
    "  { id: 't2', franchiseName: 'Baseline Kings', categories: '[\"Open\"]', isLate: false, players: [{ id: 'p3', name: 'Yuki Tanaka', email: 'yuki@play.com' }, { id: 'p4', name: 'Felix Braun', email: 'fbraun@play.com' }] },",
    "  { id: 't3', franchiseName: 'Net Ninjas', categories: '[\"Open\",\"35+\"]', isLate: true, players: [{ id: 'p5', name: 'Amara Diallo', email: 'amara@play.com' }] },",
    "  { id: 't4', franchiseName: 'Volley Vanguards', categories: '[\"35+\"]', isLate: false, players: [{ id: 'p6', name: 'Samuel Torres', email: 's.torres@play.com' }, { id: 'p7', name: 'James Omollo', email: 'jomollo@play.com' }] },",
    "  { id: 't5', franchiseName: 'Ace Hunters', categories: '[\"Open\"]', isLate: false, players: [{ id: 'p8', name: 'Priya Menon', email: 'priya@play.com' }, { id: 'p9', name: 'Lea Fischer', email: 'lea@play.com' }] },",
    "  { id: 't6', franchiseName: 'Smash Bros FC', categories: '[\"U21\"]', isLate: false, players: [{ id: 'p10', name: 'Omar Said', email: 'omar@play.com' }, { id: 'p11', name: 'Nico Haber', email: 'nhaber@play.com' }] },",
    "];",
    "",
    "const MOCK_STATS = { pct: 42, done: 8, total: 19, avgSec: 2940 };",
    "",
    ...
    "",
    "function groupByCategory(teams: typeof MOCK_TEAMS) {",
    "  const map: Record<string, typeof MOCK_TEAMS> = {};",
    "  teams.forEach((t) => {",
    ...
    "}",
    "",
    "export default function SandboxHostDashboard() {",
    "  const [phase, setPhase] = React.useState<Phase>('EARLY');",
    "  const [copied, setCopied] = React.useState(false);",
    "",
    "  const magicLink = 'https://tennissuite.app/register?t=' + MOCK_TOURNAMENT.id;",
    ...
    "  const cats = groupByCategory(MOCK_TEAMS);",
    "  const fmt = (s: number) => Math.floor(s / 60) + 'm ' + (s % 60) + 's';",
    "  const cyclePhase = () => { const o: Phase[] = ['EARLY', 'LATE', 'CLOSED']; setPhase((p) => o[(o.indexOf(p) + 1) % o.length]); };",
    "  const handleCopy = () => { navigator.clipboard.writeText(magicLink).catch(() => {}); setCopied(true); setTimeout(() => setCopied(false), 2000); };",
    "",
    "  const statCards = [",
    "    { label: 'Completion', value: MOCK_STATS.pct + '%', color: '#58a6ff' },",
    "    { label: 'Matches', value: MOCK_STATS.done + ' / ' + MOCK_STATS.total, color: '#3fb950' },",
    "    { label: 'Avg Duration', value: fmt(MOCK_STATS.avgSec), color: '#d2a8ff' },",
    "    { label: 'Teams', value: String(MOCK_TEAMS.length), color: '#f5a623' },",
    "  ];",
    "",
    "  const S = {",
    ...
    "  return (",
    "    <div style={S.page}>",
    "      <div style={{ marginBottom: '20px' }}><span style={S.pill}>SANDBOX - Host Dashboard Preview</span></div>",
    "",
    "      <header style={S.header}>",
    "        <div>",
    "          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '8px', flexWrap: 'wrap' }}>",
    "            <h1 style={S.h1}>{MOCK_TOURNAMENT.name}</h1>",
    "            <Badge variant={PHASE_VARIANTS[phase]}>{PHASE_LABELS[phase]}</Badge>",
    "          </div>",
    "          <p style={S.muted}>{MOCK_TOURNAMENT.location} | ID: <span style={{ fontFamily: 'monospace', color: '#58a6ff' }}>{MOCK_TOURNAMENT.id}</span></p>",
    "        </div>",
    "        <div>",
    "          {phase === 'CLOSED' && <Button variant='success' onClick={cyclePhase}>Open Early Registration</Button>}",
    "          {phase === 'EARLY' && <Button variant='secondary' onClick={cyclePhase}>Switch to Late Onsite Reg</Button>}",
    "          {phase === 'LATE' && <Button variant='secondary' onClick={cyclePhase}>Close Registration</Button>}",
    "        </div>",
    "      </header>",
    "",
    ...
    "            </div>",
    "            <Button variant='primary' disabled={phase === 'EARLY'}>Enter Pools Workspace</Button>",
    "          </div>",
    "        </Card>",
    "",
    ...
    "            </div>",
    "            <Button variant='primary'>Enter Dispatcher Workspace</Button>",
    "          </div>",
    "        </Card>",
    "",
    ...
    "            Choose CSV file<input type='file' accept='.csv' style={{ display: 'none' }} />",
    "          </label>",
    "        </Card>",
    "      </div>",
    "",
    ...
    "        <span style={{ fontSize: '0.9rem', color: '#8b949e', fontWeight: 500, marginLeft: '12px' }}>{MOCK_TEAMS.length} teams registered</span>",
    "      </h2>",
    "",
    ...
    "                        {t.isLate && <Badge variant='warning'>LATE</Badge>}",
    "                      </div>",
...
with open(path, "w", encoding="utf-8", newline="\n") as f:
    f.write(content)
print(f"Written {len(content)} bytes OK")
